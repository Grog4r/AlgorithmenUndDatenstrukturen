	...
	public static void main(String[] args) throws Exception {
		if (args.length > 0) {
			...
			if (!algo.equals("-aus") && !algo.equals("-part")) {
				// System.out.println("args[0]: \"" + algo + "\"");
				throw new Exception("Der erste Parameter muss entweder -part für "
						+ "den Partitionierungsalgorithmus sein, oder " + "-aus für den Auswaehlen Algorithmus.");
			} else if (algo.equals("-aus")) {

				for (int i = 2; i < args.length; i++) {
					if (Integer.parseInt(args[i]) < 0 || Integer.parseInt(args[i]) > 9) {
						throw new Exception("Die Werte des Arrays müssen zwischen 0 und 9 sein.");
					}
				}
				int k = 0;
				if (args.length >= 2) {
					...
					throw new Exception("k muss als zweites Argument angegeben werden!");
				}
				if (k > args.length - 2 || k < 1) {
					throw new Exception(
							"k muss mindestens 1 sein und darf nicht größer sein als die länge des Arrays.");
				}

				int sizeArray = args.length - 2;
				int[] inputArray = new int[sizeArray];
				for (int i = 2; i < args.length; i++) {
					inputArray[i - 2] = Integer.parseInt(args[i]);
				}
				printParameters(algo, inputArray, k);
				Auswaehlen.auswaehlen(inputArray, k, 0, inputArray.length - 1, true, true);

			} else if (algo.equals("-part")) {
				// Array Werte einstellig
				...
				int sizeArray = args.length - 1;
				int[] inputArray = new int[sizeArray];
				for (int i = 1; i < args.length; i++) {
					inputArray[i - 1] = Integer.parseInt(args[i]);
				}
				printParameters(algo, inputArray, 0);
